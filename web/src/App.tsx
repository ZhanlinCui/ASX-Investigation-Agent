...
type ProviderDiagnostic = { provider: string; operation: string; retrieved_at: string; source_version?: string; artifact_id?: string };
type Checkpoint = { stage: string; created_at: string; policy_version: string };
type Assertion = { assertion_id: string; evidence_id: string; exact_text: string; span_hash: string; artifact_hash: string; locator?: string; role: string; causal_eligible: boolean; mechanism_hint: string };
...
export type Report = {
  ...
  provider_diagnostics: ProviderDiagnostic[]; artifact_hashes: string[]; checkpoint_lineage: Checkpoint[];
...
export function ReportView({ report, onRefined }: { report: Report; onRefined: (caseId: string) => void }) {
  ...
  return <div className="report-stack">
    ...
    <details className="trace-card"><summary>Provenance</summary><div><p>Artifact SHA-256 values identify frozen provider responses; raw artifact content is never shown here.</p>{report.provider_diagnostics.map((item, index) => <span key={`${item.provider}-${item.operation}-${index}`}>{item.provider} · {human(item.operation)} · {new Date(item.retrieved_at).toLocaleString("en-AU", { timeZone: "Australia/Sydney" })} · {item.source_version ?? "source version unavailable"} · {item.artifact_id ?? "no artifact"}</span>)}{report.checkpoint_lineage.map((item) => <span key={`${item.stage}-${item.created_at}`}>Checkpoint {human(item.stage)} · {item.policy_version}</span>)}</div></details>
